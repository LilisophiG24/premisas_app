...
# ── Directorio persistente para bases de datos ───────────────────────────────
# En Railway: montar un volumen en /app/data y las DBs sobreviven entre deploys.
# Localmente: usa el directorio del script (comportamiento anterior).
_DB_DIR = os.environ.get("CND_DATA_DIR") or os.path.join(os.path.dirname(os.path.abspath(__file__)), "data")
os.makedirs(_DB_DIR, exist_ok=True)

def _db(nombre):
    """Devuelve la ruta completa de una base de datos en el directorio persistente."""
    return os.path.join(_DB_DIR, nombre)
...
def init_databases():
    ...
    # Tabla de Unidades
    with sqlite3.connect(_db('unidades_reporte.db')) as conn:
        conn.execute("""
    ...
    with sqlite3.connect(_db('rtr_seguimiento.db')) as conn:
        conn.execute("""
    ...
    with sqlite3.connect(_db('vertimiento.db')) as conn:
        conn.execute("""
    ...
    with sqlite3.connect(_db('eventos_asep.db')) as conn:
        conn.execute("""
    ...
    with st.expander("➕ Configuración de Seguimiento (Base de Datos)", expanded=True):
        ...
        if st.button("💾 Registrar / Actualizar Base de Datos", help="Agrega de forma manual una nueva línea"):
            if n_linea and n_desc:
                conn = sqlite3.connect(_db('rtr_seguimiento.db'))
...
def vista_pan(archivo_bitacora):
    ...
    # CSS para limitar el ancho de los expanders de control
    st.markdown("""
    <style>
    div[data-testid="stExpander"]:has(summary span:contains("Cargar Reporte")),
    div[data-testid="stExpander"]:has(summary span:contains("Agregar evento")) {
        max-width: 520px;
    }
    </style>
    """, unsafe_allow_html=True)

    # ── Controles superiores (col izquierda estrecha) ────────────────────────
    col_ctrl, col_space = st.columns([2, 3])
    with col_ctrl:

        # 1. Cargar reporte ya generado
        with st.expander("📂 Cargar Reporte EOR ya generado", expanded=False):
            st.caption("Sube un EOR generado anteriormente para continuar editándolo.")
            ...
            if uploaded_pan_prev is not None:
                try:
                    ...
                    if _eventos_cargados:
                        ...
                        st.success(f"✅ {len(_eventos_cargados)} evento(s) cargados.")
                    else:
                        st.error("No se encontraron eventos. El archivo debe tener columnas Hora y Descripción.")
                except Exception as _e:
                    st.error(f"No se pudo leer el archivo: {_e}")
            if st.session_state.get("pan_from_upload"):
                if st.button("↩️ Volver a procesar desde bitácora", key="pan_clear_upload"):
                    st.session_state.pop("pan_from_upload", None)
                    st.session_state.pop("pan_eventos", None)
                    st.session_state.pop("pan_archivo_id", None)
                    st.rerun()

        # 2. Agregar evento manualmente
        with st.expander("➕ Agregar evento manualmente", expanded=False):
            _c1, _c2, _c3 = st.columns([1, 3, 1])
            hora_nueva = _c1.text_input("Hora (HH:MM)", placeholder="07:45", key="pan_hora_nueva")
            desc_nueva = _c2.text_input("Descripción", placeholder="Descripción del evento", key="pan_desc_nueva")
            if _c3.button("Agregar", type="primary", use_container_width=True, key="pan_btn_agregar_top"):
                ...
                if not hora_nueva or not desc_nueva:
                    ...
                    st.warning("Formato de hora: HH:MM (ej. 07:45).")
                else:
                    evs_act = st.session_state.get("pan_eventos", [])
                    evs_act.append({"Hora": hora_nueva, "Descripción": desc_nueva.upper()})
                    evs_act.sort(key=lambda x: x["Hora"])
                    st.session_state["pan_eventos"] = evs_act
                    st.rerun()

        # 3. Forzar reprocesamiento
        if st.button("🔄 Forzar reprocesamiento desde bitácora", key="pan_forzar",
                     use_container_width=True,
                     help="Descarta el reporte actual y vuelve a extraer eventos desde la bitácora"):
            st.session_state.pop("pan_archivo_id", None)
            st.session_state.pop("pan_from_upload", None)
    ...
    try:
        ...
        st.markdown("#### 📋 Eventos del Reporte")
        st.caption("Puedes editar las celdas directamente, marcar filas para eliminarlas, o usar IA para mejorar la redacción.")
...
_PLANILLA_NUM_EMPLEADOS = {
    ...
    'W': 27863, 'X': 28034, 'Y': 27836, 'Z': 27714,
}
_PLANILLA_CEDULAS = {
...
# Empleados con planilla de gestión (formato Horas Extras1.0 / Autorización)
# Jornada ordinaria 07:00-15:30, sin turnos rotativos
_PLANILLA_GESTION_CODS = {'X', 'W'}   # X = Mónica Sánchez · W = Jair Córdoba
_PLANILLA_GESTION_PERFIL = {
    # cod: (cargo, unidad_administrativa, jornada_ordinaria)
    'X': ('Analista de Operaciones de la Red', 'Despacho de Energía', '07:00h - 15:30h'),
    'W': ('Analista de Operaciones de la Red', 'Despacho de Energía', '07:00h - 15:30h'),
}
...
def _fetch_horario_planilla(nombre_archivo):
    """Lee archivo de horario desde archivo local."""
    import os
    if os.path.exists(nombre_archivo):
        with open(nombre_archivo, "rb") as f:
            return f.read(), None
    return None, f"No se encontró el archivo {nombre_archivo} en el servidor."
...
def _llenar_gasto_gestion(ws_g, nombre, num_empleado, cedula,
                          fecha, monto_prolongacion, descripcion,
                          jornada_ord_fin, ext_ini, ext_fin):
    """
    Llena una hoja 'Gasto de Alimentacion' del formato de gestión (Mónica/Jair).
    - nombre       : en mayúsculas en celda (11,5) para VLOOKUP
    - monto_prolongacion : importe en (23,17) — fila 29.3 PROLONGACIÓN ≥ 3h
    - descripcion  : texto en (30,2)
    - jornada_ord_fin : hora fin jornada ordinaria como string "HH:MM" → celda (35,15)
    - ext_ini / ext_fin : horario extraordinario → celdas (35,25) y (35,30)
    - fecha         : día/mes/año → celdas (36,9), (36,11), (36,14)
    """
    _MESES_G = {1:'Enero',2:'Febrero',3:'Marzo',4:'Abril',5:'Mayo',6:'Junio',
                7:'Julio',8:'Agosto',9:'Septiembre',10:'Octubre',11:'Noviembre',12:'Diciembre'}
    ...
    ws_g.cell(11, 5).value = nombre.upper()
    if monto_prolongacion:
        ws_g.cell(23, 17).value = monto_prolongacion
    ws_g.cell(30, 2).value = descripcion

    # Detalle del horario
    if jornada_ord_fin:
        ws_g.cell(35, 15).value = _str_to_time(jornada_ord_fin)
    if ext_ini:
        ws_g.cell(35, 25).value = _str_to_time(ext_ini)
    if ext_fin:
        ws_g.cell(35, 30).value = _str_to_time(ext_fin)
    ws_g.cell(36, 9).value  = fecha.day
    ws_g.cell(36, 11).value = _MESES_G[fecha.month]
    ws_g.cell(36, 14).value = str(fecha.year)


def generar_planilla_gestion(cod, nombre, num_empleado, cedula,
                             periodo_inicio, periodo_fin,
                             eventos_extra=None):
    """
    Genera la planilla formato gestión (Horas Extras1.0 / Autorización / Gasto de Alimentacion)
    para empleados con jornada ordinaria fija (Mónica Sánchez, Jair Córdoba, etc.).

    eventos_extra: lista de dicts con:
      tipo='extendida': {tipo, fecha, horas, minutos, justificacion, hora_inicio (opt)}
    """
    from datetime import time as dtime

    _base = os.path.dirname(os.path.abspath(__file__))
    PLANTILLA = os.path.join(_base, "planilla_gestion.xlsx")
    if not os.path.exists(PLANTILLA):
        PLANTILLA = os.path.join(os.getcwd(), "planilla_gestion.xlsx")
    if not os.path.exists(PLANTILLA):
        return None, "No se encontró 'planilla_gestion.xlsx' en la carpeta de la app."

    perfil = _PLANILLA_GESTION_PERFIL.get(cod, ('', 'Despacho de Energía', '07:00h - 15:30h'))
    cargo, unidad, jornada_ord_str = perfil
    # Hora fin de jornada ordinaria (para Gasto)
    jord_fin = jornada_ord_str.split('-')[-1].replace('h','').strip()  # "15:30"
    jord_ini = jornada_ord_str.split('-')[0].replace('h','').strip()   # "07:00"
    ...
    # ── Hoja Autorización — nombre + filas de eventos ──────────────────────
    ws_a = wb['Autorización']
    ws_a.cell(7, 2).value = nombre

    evs = [ev for ev in (eventos_extra or []) if ev['tipo'] == 'extendida']
    for idx, ev in enumerate(evs[:10]):   # máximo 10 filas
        fila = 9 + idx
        hora_inicio = ev.get('hora_inicio') or jord_fin  # por defecto fin jornada ordinaria
        ext_min = ev['horas'] * 60 + ev['minutos']
        h_fin_prog = _str_to_time(hora_inicio)
        # hora final programada = inicio + extensión
        if h_fin_prog:
            total_min = h_fin_prog.hour * 60 + h_fin_prog.minute + ext_min
            h_fin_calc = dtime((total_min // 60) % 24, total_min % 60)
        else:
            h_fin_calc = None
        fecha_dt = datetime.combine(ev['fecha'], datetime.min.time())
        ws_a.cell(fila, 1).value = fecha_dt
        ws_a.cell(fila, 1).number_format = 'DD/MM/YYYY'
        ws_a.cell(fila, 2).value = _str_to_time(hora_inicio)
        ws_a.cell(fila, 3).value = h_fin_calc
        ws_a.cell(fila, 4).value = h_fin_calc   # ejecutada = programada (editar manualmente si difiere)
        ws_a.cell(fila, 5).value = f'=D{fila}-B{fila}'
        ws_a.cell(fila, 6).value = ev.get('justificacion', '')

    # ── Hoja Horas Extras1.0 — datos del empleado ─────────────────────────
    ws_h = wb['Horas Extras1.0']
    ws_h.cell(13, 15).value = nombre
    ws_h.cell(12, 42).value = cedula
    ws_h.cell(13, 42).value = cargo
    ws_h.cell(14, 15).value = f"04-50-01-01-{num_empleado}" if num_empleado else ""
    ws_h.cell(14, 44).value = periodo_inicio
    ws_h.cell(14, 44).number_format = 'DD/MM/YYYY'
    ws_h.cell(15, 15).value = unidad
    ws_h.cell(16, 15).value = jornada_ord_str

    # ── Hojas Gasto de Alimentacion — una por jornada extendida ≥ 3h ──────
    evs_gasto = [ev for ev in evs if ev['horas'] * 60 + ev['minutos'] >= 180]
    nombres_gasto = [s for s in wb.sheetnames if 'Gasto de Alimentacion' in s]
    ws_gasto_base = wb[nombres_gasto[0]]

    # Crear hojas adicionales si hacen falta
    gasto_sheets = [ws_gasto_base]
    for i in range(1, len(evs_gasto)):
        new_ws = wb.copy_worksheet(ws_gasto_base)
        new_ws.title = f'Gasto de Alimentacion ({i+1})'
        gasto_sheets.append(new_ws)
    # Renombrar la primera
    ws_gasto_base.title = 'Gasto de Alimentacion (1)'

    for ws_g, ev in zip(gasto_sheets, evs_gasto):
        hora_inicio = ev.get('hora_inicio') or jord_fin
        ext_min = ev['horas'] * 60 + ev['minutos']
        h_ini_t = _str_to_time(hora_inicio)
        if h_ini_t:
            total_min = h_ini_t.hour * 60 + h_ini_t.minute + ext_min
            h_fin_str = f"{(total_min // 60) % 24:02d}:{total_min % 60:02d}"
        else:
            h_fin_str = None
        fecha_dt = datetime.combine(ev['fecha'], datetime.min.time())
        _llenar_gasto_gestion(
            ws_g, nombre, num_empleado, cedula,
            fecha_dt,
            monto_prolongacion=10,
            descripcion=ev.get('justificacion', ''),
            jornada_ord_fin=jord_fin,
            ext_ini=hora_inicio,
            ext_fin=h_fin_str,
        )

    # Eliminar hojas de gasto sobrantes si hay más plantillas que eventos
    hojas_gasto_actuales = [s for s in wb.sheetnames if 'Gasto de Alimentacion' in s]
    for nombre_hoja in hojas_gasto_actuales[len(evs_gasto):]:
        del wb[nombre_hoja]
...
def _fetch_horario_github():
    """Lee el horario desde archivo local."""
    import os
    fname = "Horario.xlsx"
    if os.path.exists(fname):
        with open(fname, "rb") as f:
            return f.read(), fname
    return None, f"No se encontró el archivo {fname} en el servidor."
        ...
        if not periodos_pl:
            ...
            if not periodo_pl["horario"]:
                ...
                if emps_pl:
                    ...
                    eventos_extra_list = []
                    es_gestion = emp_pl['cod'] in _PLANILLA_GESTION_CODS
                    _ek = f"{emp_pl['cod']}_{_per_key}"

                    if es_gestion:
                        # ── Formato gestión (Mónica / Jair): solo jornada extendida ──
                        perfil_g = _PLANILLA_GESTION_PERFIL.get(emp_pl['cod'], ('', '', '07:00h - 15:30h'))
                        st.info(f"🗂️ Planilla formato **Gestión** · Jornada ordinaria: **{perfil_g[2]}**")

                        st.markdown("**⏱️ Jornadas extendidas del período**")
                        hay_ext_g = st.checkbox("¿Hubo jornada(s) extendida(s)?", key=f"pl_g_hay_ext_{_ek}")
                        if hay_ext_g:
                            n_ext_g = st.number_input("Número de jornadas extendidas",
                                                      min_value=1, max_value=20, value=1, step=1,
                                                      key=f"pl_g_n_ext_{_ek}")
                            for _ne in range(int(n_ext_g)):
                                cg1, cg2, cg3, cg4, cg5 = st.columns([2, 1, 1, 1, 3])
                                _fext = cg1.date_input(
                                    f"Fecha {_ne+1}",
                                    value=p_ini.date(), min_value=p_ini.date(), max_value=p_fin.date(),
                                    key=f"pl_g_ext_fecha_{_ek}_{_ne}",
                                )
                                _h_ini_def = perfil_g[2].split('-')[-1].replace('h','').strip()
                                _h_ini_str = cg2.text_input("Inicio ext.",
                                                             value=_h_ini_def,
                                                             key=f"pl_g_ext_hini_{_ek}_{_ne}",
                                                             placeholder="15:30")
                                _hext = cg3.number_input("Horas", min_value=0, max_value=12,
                                                         value=0, step=1, key=f"pl_g_ext_h_{_ek}_{_ne}")
                                _mext = cg4.number_input("Min", min_value=0, max_value=59,
                                                         value=0, step=5, key=f"pl_g_ext_m_{_ek}_{_ne}")
                                _jext = cg5.text_input(f"Descripción / tarea {_ne+1}",
                                                       key=f"pl_g_ext_just_{_ek}_{_ne}",
                                                       placeholder="Revisión de libranzas ETESA-XXX...")
                                if int(_hext) > 0 or int(_mext) > 0:
                                    eventos_extra_list.append({
                                        'tipo':        'extendida',
                                        'fecha':       _fext,
                                        'hora_inicio': _h_ini_str.strip(),
                                        'horas':       int(_hext),
                                        'minutos':     int(_mext),
                                        'justificacion': _jext,
                                    })
                                    _total_ext = int(_hext) * 60 + int(_mext)
                                    if _total_ext >= 180:
                                        st.caption(f"✅ {int(_hext)}h {int(_mext)}min ≥ 3h — se generará **Gasto de Alimentación $10.00**.")
                                    else:
                                        _faltan = 180 - _total_ext
                                        st.caption(f"ℹ️ Faltan {_faltan//60}h {_faltan%60}min para generar gasto alimenticio.")

                    else:
                        # ── Formato operaciones (turnos rotativos) ────────────────
                        st.markdown("**🔁 Doble jornada**")
                        hay_doble = st.checkbox("¿Hubo doble jornada en el período?", key=f"pl_hay_doble_{_ek}")
                        if hay_doble:
                            n_dobles_pl = st.number_input("Cantidad de dobles en el período",
                                                          min_value=1, max_value=5, value=1, step=1,
                                                          key=f"pl_n_dobles_{_ek}")
                            for _nd in range(int(n_dobles_pl)):
                                cd1, cd2 = st.columns([1, 2])
                                _fd = cd1.date_input(
                                    f"Fecha doble {_nd+1}",
                                    value=p_ini.date(), min_value=p_ini.date(), max_value=p_fin.date(),
                                    key=f"pl_doble_fecha_{_ek}_{_nd}",
                                )
                                _jd = cd2.text_input(
                                    f"Justificación doble {_nd+1}",
                                    key=f"pl_doble_just_{_ek}_{_nd}",
                        ...
                        if hay_ext:
                            ...
                            for _ne in range(int(n_ext)):
                                ...
                                if int(_hext) > 0 or int(_mext) > 0:
                                    ...
                                    _total_ext = int(_hext) * 60 + int(_mext)
                                    if _total_ext >= 180:
                                        st.caption(f"✅ {int(_hext)}h {int(_mext)}min ≥ 3h — se generará gasto de alimentación de **$10.00** (prolongación de jornada).")
                                    else:
                                        _faltan = 180 - _total_ext
                                        st.caption(f"ℹ️ {int(_hext)}h {int(_mext)}min — faltan {_faltan // 60}h {_faltan % 60}min para alcanzar las 3h que generan gasto alimenticio.")

                        # ── Cambio de turno ───────────────────────────────────────
                        st.markdown("**🔄 Cambio de turno**")
                        hay_cambio = st.checkbox("¿Hubo cambio de turno en el período?", key=f"pl_hay_cambio_{_ek}")
                        if hay_cambio:
                            ...
                            n_cambios = st.number_input("Cantidad de cambios de turno", min_value=1, max_value=5,
                                                        value=1, step=1, key=f"pl_n_cambio_{_ek}")
                            for _nc in range(int(n_cambios)):
                                cc1, cc2, cc3 = st.columns([2, 1, 2])
                                _fc = cc1.date_input(
                                    f"Fecha cambio {_nc+1}",
                                    value=p_ini.date(), min_value=p_ini.date(), max_value=p_fin.date(),
                                    key=f"pl_cambio_fecha_{_ek}_{_nc}",
                                )
                                _tc = cc2.selectbox(
                                    f"Turno trabajado",
                                    ["07-15", "15-23", "23-07"],
                                    key=f"pl_cambio_turno_{_ek}_{_nc}",
                                )
                                _jc = cc3.text_input(
                                    f"Justificación {_nc+1}",
                                    key=f"pl_cambio_just_{_ek}_{_nc}",
                    ...
                    if st.button("📄 Generar Planilla", type="primary", key="btn_gen_planilla"):
                        with st.spinner("Procesando..."):
                            if es_gestion:
                                datos_pl, err_pl = generar_planilla_gestion(
                                    emp_pl["cod"], emp_pl["nombre"],
                                    str(emp_pl["num_empleado"]), emp_pl.get("cedula", ""),
                                    p_ini, p_fin,
...
def _parsear_eventos_asep(df_bit):
    ...
    def _extraer_texto_circuitos(tnj):
        ...
        # Formato 0: CTOS AFECTADO/AFECTADOS X, Y, Z CARGA: (esta bitácora)
        m = re.search(r'CTOS\s+AFECTAD[AO]S?\s+([A-Z0-9,\s\.\-Y]+?)\s*(?:,?\s*CARGA|INFORMA|CONFIRMA|$)', tnj)
        ...
        if m: return m.group(1)
        # Formato 2: CTOS: X, Y, Z  (abreviación usada por ENSA con dos puntos)
        m = re.search(r'CTOS\s*:\s*([A-Z0-9,\s\.\-]+?)(?:\s*\(\d|\s+Y\s+\d\s+ESCALON|\s+MONTO|\s+INFORMA|$)', tnj)
        if m: return m.group(1)
        # Formato 3: 1ER/PRIMER ESCALON= X, Y, Z
        m = re.search(
    ...
    def _extraer_texto_circuitos_2do(tnj):
        """Extrae el texto de lista de circuitos del 2do escalon."""
        m = re.search(
            r'(?:2DO|SEGUNDO)\s+ESCALON\s+(?:FUERON|SON|ACTUARON[^=]*)[=:\s]*'
            r'([A-Z0-9,\s\.\-]+?)(?:\.|$)',
            tnj
        )
        if m: return m.group(1)
        m = re.search(
            r'[/,]\s*(?:CARGA\s+TOTAL\s+DEL\s+)?(?:2DO|SEGUNDO)\s+ESCALON\s*[=:]\s*'
            r'[\d\.]+\s*MW[^/]*'
            r'CIRCUITOS[^=]*[=:\s]+([A-Z0-9,\s\.\-]+?)(?:\.|$)',
            tnj
        )
        if m: return m.group(1)
        return None

    def _sumar_mw_escalones(tnj):
        """Suma los MW de TODOS los escalones / CARGA mencionados en el texto BF."""
        # Formato: CARGA TOTAL DEL Ndo ESCALON= X.XX MW
        mws = re.findall(
            r'(?:CARGA\s+TOTAL\s+DEL\s+)?(?:\d[ERD]+O?|PRIMER|SEGUNDO|TERCER)\s+ESCALON\s*[=:]\s*'
            r'(\d+(?:\.\d+)?)\s*MW',
            tnj
        )
        if mws:
            return sum(float(m) for m in mws)
        # Formato: CARGA: X.XXMW  (esta bitácora)
        mws2 = re.findall(r'CARGA\s*:\s*(\d+(?:\.\d+)?)\s*MW', tnj)
        if mws2:
            return sum(float(m) for m in mws2)
        # Fallback: primer MW
        m = re.search(r'(\d+(?:\.\d+)?)\s*MW', tnj)
    ...
    for grp in grupos:
        ...
        for j in range(j_start, j_end):
            ...
            # Saltar filas ACTIVADO que pertenecen a un evento diferente
            # (mismo tipo de esquema pero en una hora distinta al grupo actual)
            if 'ACTIVADO' in tnj and any(k in tnj for k in ['ESQUEMA BAJA FRECUENCIA','ESQUEMA BAJO VOLTAJE','SPEAR','BLOQUE DE CARGA']):
                es_de_este_grupo = any(abs(h_min_j - _hmin(t['hora'])) <= 2 for t in grp)
                if not es_de_este_grupo:
                    continue
            ...
            # ESQUEMA BAJA FRECUENCIA
                ...
                mw = _sumar_mw_escalones(tnj)
                circ_txt = _extraer_texto_circuitos(tnj)
                circs = _extraer_circuitos_bf(circ_txt) if circ_txt else []
                circ_txt_2 = _extraer_texto_circuitos_2do(tnj)
                if circ_txt_2:
                    circs += _extraer_circuitos_bf(circ_txt_2)
                area_str = _extraer_areas(tnj)
            ...
            # Solo dentro de 30 min del trigger (evita capturar el normalizado del evento siguiente)
            if 'NORMALIZADO' in tnj and m_h:
                if any(k in tnj for k in ['ESQUEMA BAJA FRECUENCIA','ESQUEMA BAJO VOLTAJE','SPEAR']):
                    h_n = m_h.group(1)
                    if _hmin(h_n) <= h_ini_min + 30:
